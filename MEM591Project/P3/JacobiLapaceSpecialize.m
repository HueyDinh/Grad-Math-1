function x = JacobiLapaceSpecialize(b,tol, true_sol)
...
while true
    ...
    for ele_index = 1:dim
        ...
        if i_minus_one_index < 1
            ...
            add_i_minus_one = x_prev(i_minus_one_index);
        end

        if i_plus_one_index > dim
            add_i_plus_one = 0;
        else
            add_i_plus_one = x_prev(i_plus_one_index);
        end

        if j_minus_one_mod > ele_index_mod
            add_j_minus_one = 0;
        else
            add_j_minus_one = x_prev(j_minus_one_index);
        end

        if j_plus_one_mod < ele_index_mod
            add_j_plus_one = 0;
        else
            add_j_plus_one = x_prev(j_plus_one_index);
        end
